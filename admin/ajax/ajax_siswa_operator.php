...
if($_GET['action'] == "table_data"){
   ...
   while($r = mysqli_fetch_array($query)){
      ...
      $row[] = $kelas['kelas'];
      $row[] = $status;
      $row[] = '<a class="btn btn-danger" onclick="reset_login(\''.$r['nis'].'\')"><i class="glyphicon glyphicon-off"></i> Reset Login</a>';
      $data[] = $row;
      $no++;
   }
...
elseif($_GET['action'] == "reset_login"){
   mysqli_query($mysqli, "UPDATE siswa set status='off' WHERE nis='$_GET[nis]'");
}

elseif($_GET['action'] == "reset_siswa"){
